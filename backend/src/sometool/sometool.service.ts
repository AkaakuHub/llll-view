...
export class SometoolService {
	...
	private buildLogSection(outputLog: string, summarizeLog: boolean): string {
		if (!summarizeLog) {
			return `Log:\n${outputLog}`;
		}
		return `Log Summary:\n${this.buildLogSummary(outputLog)}`;
	}

	private buildLogSummary(outputLog: string): string {
		const updatedEntries = this.extractUpdatedEntries(outputLog);
		...
		for (const entry of updatedEntries) {
			const category = this.toReportedCategory(
				this.detectTentativeCategory(entry),
			);
			counts[category] += 1;
		}
	...
	private extractUpdatedEntries(outputLog: string): string[] {
	...
	private detectTentativeCategory(entry: string): TentativeLogCategory {
